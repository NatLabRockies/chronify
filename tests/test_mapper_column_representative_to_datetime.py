...
@pytest.fixture
def iter_store(iter_stores_by_backend):
    return iter_stores_by_backend
...
def test_MDH_mapper(time_series_NMDH, iter_store: Store):
    ...
    map_time(iter_store._backend, from_schema, to_schema, check_mapped_timestamps=True)

    table = iter_store._backend.table(to_schema.name)
    mapped_table = read_query(iter_store._backend, table, to_schema.time_config).sort_values(
        "timestamp"
    )
